...
export const generateMockChartData = (
  ...
  for (let i = 0; i <= totalDays; i++) {
    ...
    const seasonalMultiplier =
      month === 11 || month === 0 || month === 6 || month === 7 ? 0.8 : 1.0

    // Random daily variation
    const randomVariation = 0.8 + Math.random() * 0.4 // 80% to 120%

    const totalMultiplier =
      weekendMultiplier * midWeekBoost * seasonalMultiplier * randomVariation

    data.push({
      date: currentDate.toISOString(),
      formattedDate: format(currentDate, 'MMM dd'),
      whole_blood: Math.round(
        baseInventory.whole_blood * totalMultiplier +
          (Math.random() - 0.5) * 30,
      ),
      red_blood_cells: Math.round(
        baseInventory.red_blood_cells * totalMultiplier +
          (Math.random() - 0.5) * 40,
      ),
      platelets: Math.round(
        baseInventory.platelets * totalMultiplier + (Math.random() - 0.5) * 20,
      ),
      fresh_frozen_plasma: Math.round(
        baseInventory.fresh_frozen_plasma * totalMultiplier +
          (Math.random() - 0.5) * 25,
      ),
      cryoprecipitate: Math.round(
        baseInventory.cryoprecipitate * totalMultiplier +
          (Math.random() - 0.5) * 15,
      ),
      albumin: Math.round(
        baseInventory.albumin * totalMultiplier + (Math.random() - 0.5) * 20,
      ),
    })
  }
